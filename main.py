...
class App(ctk.CTk):
    def __init__(self):
        ...
        self._res_autoset_done = False # scale min-defect-size to camera resolution on first frame
        self.static_name = ""
    ...
    def _load_image_path(self, path):
        ...
        self._res_autoset_done = False   # re-evaluate min-area for this image's resolution
    ...
    # ========================================================= render loop
    def _maybe_autoset_for_resolution(self, frame):
        """Once, on the first frame, pick a sane min-defect-size for the
        actual camera resolution. A 6px floor is fine at ~1MP but on a
        20MP (5472x3648) frame the coating's own grain forms 6-50px
        clusters and floods the result with fake detects; real dust there
        is 100+ px. Only applies when no saved model is loaded (a loaded
        config's own value always wins)."""
        if self._res_autoset_done:
            return
        self._res_autoset_done = True
        if self.current_model.get().strip():
            return  # a model config is active; respect its saved value
        long_side = max(frame.shape[0], frame.shape[1])
        scale = long_side / 1280.0
        if scale <= 1.2:
            return  # already near the reference resolution, defaults are fine
        self.min_area.set(int(round(6 * scale * scale)))   # area scales as resolution^2
        self._refresh_param_labels()
        self.save_status_label.configure(
            text=f"\u2699 {long_side}px camera detected - min defect size set to "
                 f"{int(self.min_area.get())}px. Tune in the Lab.")
        self.after(6000, lambda: self.save_status_label.configure(text=""))

    def _tick(self):
        frame = self.static_image if self.static_image is not None else self.camera.get_frame()
        if frame is not None:
            self._maybe_autoset_for_resolution(frame)
            display = frame.copy()
            for i, r in enumerate(self.rois):
